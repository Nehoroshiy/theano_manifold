...
from manifolds import FixedRankEmbeeded, ManifoldElementShared, TangentVectorShared
...
class DotStep(theano.gof.Op):
    ...
    def perform(self, node, inputs, output_storage):
        ...
        U, S, V = [field.get_value(borrow=True) for field in [w.U, w.S, w.V]]
    ...
    def grad(self, input, output_gradients):
        ...
        xin_grad = out_grad.dot(w.V.T).dot(w.S.T).dot(w.U.T).reshape(xin.shape)
    ...
    * ``param := param - learning_rate * gradient``
    ...
    for param, grad in zip(params, grads):
        if "fixed_rank" in param.name:
            updates[param] = FixedRankEmbeeded(*param.shape, param.r).retr(param, -learning_rate * grad)
        else:
